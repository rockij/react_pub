...
- 컴포넌트를 이용해 Screen Case를 추가할 때 CSS는 `src/assets/css/screen` 경로에 케이스별 파일로 추가해 작업합니다.
...
- 개인 브랜치에서 작업 후 push 전에는 페이지에 빨간줄이 없는지 확인하고, `npm run lint`와 `npm run build`를 실행해 에러 여부를 점검합니다.
